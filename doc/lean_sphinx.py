...
def html_visit_lean_code_goodies(self, node):
    new_node = node
    # global prev_nodes
    # prev_nodes = prev_nodes + node['full_code']
    # new_node = copy(node)
    # new_node['full_code'] = prev_nodes
    self.body.append(self.starttag(new_node, 'div', style='position: relative'))
    self.body.append("<div style='position: absolute; right: 0; top: 0; padding: 1ex'>")
    self.body.append(self.starttag(new_node, 'a', target='_blank', href=mk_try_it_uri(new_node['full_code'])))
...
class LeanTestBuilder(Builder):
    ...
    def write_doc(self, docname, doctree):
        # i = 0
        fn = os.path.join(self.outdir, '{0}.lean'.format(docname))
